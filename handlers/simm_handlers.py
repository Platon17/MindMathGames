...
# ==== TRAIN SIMM ===
@router_simm.message(WordTrainPath())
@router_simm.message(StateFilter(FSM_state.wPath),WordPath())
...
#@router_simm.message(WordSimm())
async def solve_Simm(message: Message, state: FSMContext, matrix: list, n_raw: int, n_col: int):
    results = solve_simm(simm)
    if results:
	result:=results.get['result'])
	for res in results:
...
async def wrong_simm(message: Message):
